		...
		children: [
			{
				...
				width: 55,
				className: group,
				render: (r: any) => colored(r.day, `${uid}_c`, get(r)),
			},
			{
				title: '%',
				key: `${uid}_p`,
...
const columns = computed(() => {
	...
	const take = (arr: string[]) => arr.filter(v => !used.has(v) && used.add(v));

	const resultCols = take([...results.value]);
	const zoneCols = take([...zones.value]);
	const reasonCols = take([...reasons.value]);
	const descCols = take([...descs.value]);
